...
namespace FullSolver
{
    ...
    namespace data
    {
        ...
        const double chi = 0.005/timestep_size;
    ...
    void FullMovingMesh<dim>::setup_dofs_rock ()
    {
        ...
        VectorTools::interpolate_boundary_values (dof_handler_rock, 2, BoundaryValues<dim>(),
                                                  constraints_rock, fe_rock.component_mask(velocities));
    ...
    void FullMovingMesh<dim>::assemble_system_rock ()
    {
        ...
        for (; cell!=endc; ++cell, ++pf_cell, ++phi_cell, ++vf_cell)
        {
            ...
            for (unsigned int q=0; q<n_q_points; ++q)
            {
                ...
                {
                    grad_pf_values[q] = grad_pf_values[q];
                }
                else
                {
                    const double k = 1;//pow(phi_values[q]/phi0,a);
                    grad_pf_values[q] = -1.0/(k) * grad_pf_values[q];
                    // after first step, [velocities] are u values
                    
                }
                
                for (unsigned int n=0; n<dofs_per_cell; ++n)
                {
                    symgrad_phi_u[n] = fe_values_rock[velocities].symmetric_gradient (n, q);
                    phi_u[n]         = fe_values_rock[velocities].value (n, q);
                    div_phi_u[n]     = fe_values_rock[velocities].divergence (n, q);
                    phi_p[n]         = fe_values_rock[pressure].value (n, q);
                }
...
//                                         grad_phi_values[q]* a *pow(phi_values[q],a-1.0)/(pow(phi0,a)) //div k
                                         )
                                                        * fe_values_pf.JxW(q);
                }
            }
...
//                        std::cout << pr_boundary_values[q] << std::endl;
                    }
                }
        ...
        {std::map<types::global_dof_index, double> boundary_values_flux;
            {
                ...
                std::vector<bool> componentVector(dim + 1, true); // condition is on u
        ...
        {std::map<types::global_dof_index, double> boundary_values_flux;
            {
                ...
                std::vector<bool> selected_dofs(n_dofs);
                std::set< types::boundary_id > boundary_ids;
                boundary_ids.insert(2); // BOTTOM FLUX CONDITION
...
//        solution_grad_pf.copy_from(solution_pf);
    }
