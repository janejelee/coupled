...
namespace FullSolver
{
    ...
    namespace data
    {
        ...
        const double T_0 = 10;
        const double kappa_f = 600;
        const double kappa_r = 1400;
        
        const double timestep_size = 0.0002;
    ...
    template <int dim>
    class FluxFunctionSides : public Function<dim>
    {
    public: FluxFunctionSides () : Function<dim>(dim) {}
    ...
    template <int dim>
    void FluxFunctionSides<dim>::vector_value (const Point<dim> &p, Vector<double>   &values) const
    {
        // Need grad pf.n = 0 so just set grad pf = 0.
        values(0) = 0.0;
        values(1) = 0.0;
    }
    ...
    {
    public: InitialFunction_T () : Function<dim>() {}
        virtual double value (const Point<dim>   &p, const unsigned int  component = 0) const;
    };
    ...
    void FullMovingMesh<dim>::setup_dofs_fluid ()
    {
        ...
        VectorTools::project_boundary_values_div_conforming
        (dof_handler_pf, 0, FluxFunctionSides<dim>(), 0, constraints_pf);
        ...
        for (; cell!=endc; ++cell, ++pf_cell, ++phi_cell, ++vf_cell)
        {
            ...
            for (unsigned int q=0; q<n_q_points; ++q)
            {
                ...
                for (unsigned int k=0; k<dofs_per_cell; ++k)
                {
                    ...
                    grad_phi_u[k]    = fe_values_rock[velocities].gradient (k, q);
                ...
                for (unsigned int i=0; i<dofs_per_cell; ++i)
                {
                    ...
                    {
                        local_matrix(i,j) += ( - 2*(1-phi_values[q])* //need 2 in front for sake of the symgrad operator having 1/2 in front of it
            ...
            for (unsigned int q=0; q<n_q_points; ++q)
            {
                for (unsigned int i=0; i<dofs_per_cell; ++i)
                {
                    ...
                    for (unsigned int j=0; j<dofs_per_cell; ++j)
                    {
                        ...
                        local_rhs(i) +=     phi_i_p * div_vr_values[q]
                                                * fe_values_pf.JxW(q);
                }
            }
        ...
        for (; cell!=endc; ++cell, ++vr_cell)
        {
            ...
            for (unsigned int face=0; face<GeometryInfo<dim>::faces_per_cell; ++face)
                if (cell->face(face)->at_boundary()
                    &&
                    (cell->face(face)->boundary_id() == 1)) // BC weak so need on LHS too
                {
    ...
    void FullMovingMesh<dim>::assemble_rhs_phi ()
    {
        ...
        for (; cell!=endc; ++cell, ++rock_cell, ++pf_cell, ++T_cell)
        {
            ...
            for (unsigned int q_point=0; q_point<n_q_points; ++q_point)
            {
                ...
                const double chem_c = pow(old_phi_values[q_point],0.1)*
                pow((1.0-old_phi_values[q_point]),2.2);
                const double mu_r = exp(E/R * (1.0/old_T_values[q_point] - 1.0/T_0));
                ...
                { // actual RHS
                    cell_rhs(i) += ((fe_values_phi.shape_value(i,q_point)
                                    ...
                                    *( -gamma*exp(-gamma* (pr_values[q_point] - pf_values[q_point]))
