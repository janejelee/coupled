...
namespace FullSolver
{
    ...
    {
    public: RockTopStress () : Function<dim>(dim+1) {}
        virtual void vector_value (const Point<dim> &p, Vector<double> &value) const;
    };
    
    template <int dim>
    void RockTopStress<dim>::vector_value (const Point<dim> &p, Vector<double> &values) const
    {
        values(0) = 0;
        values(1) = rocktopstress;
        values(2) = 0;
    }
    // n.(pI-2e(u)) for the top
    template <int dim>
    ...
    {
    public: RockBottomStress () : Function<dim>(dim+1) {}
        virtual void vector_value (const Point<dim> &p, Vector<double>   &value) const;
    };
    
    template <int dim>
    void RockBottomStress<dim>::vector_value (const Point<dim> &p, Vector<double> &values) const
    {
        values(0) = 0;
        values(1) = rockbottomstress;
        values(2) = 0;
    }
    ...
    void InitialFunction_pf<dim>::vector_value (const Point<dim> &p, Vector<double> &values) const
    {
        ...
        values(0) = 2*time*top*(PI/right)*cos(PI*p[0]/right); //grad pfx
        values(1) = -2*time*sin(PI*p[0]/right); //grad pfz
        values(2) = patm + 2* time *(top-p[1])*sin(PI*p[0]/right); //p
    }
    ...
    {
        const double time = this->get_time();
        return T0 + (1.0 - p[1]*p[1])*time*1e3;
    }
    ...
    template <int dim>
    class pr_known : public Function<dim>
    {
    public:
        pr_known () : Function<dim>() {}
        virtual double value (const Point<dim>   &p, const unsigned int  component = 0) const;
    };
    
    template <int dim>
    double pr_known<dim>::value (const Point<dim>  &p, const unsigned int /*component*/) const
    {
        const double time = this->get_time();
        return patm + 5*(top-p[1])*exp(-time);
    }
    
    template <int dim>
    class grad_pr_known : public TensorFunction<1,dim>
    {
    public:
        grad_pr_known () : TensorFunction<1,dim> () {}
    ...
    template <int dim>
    Tensor<1,dim>
    grad_pr_known<dim>::value (const Point<dim> &p) const
    {
        Point<dim> value;
        const double time = this->get_time();
        value(0) = 0.0;
    ...
    template <int dim>
    void
    grad_pr_known<dim>::value_list (const std::vector<Point<dim> > &points,
                                std::vector<Tensor<1,dim> >    &values) const
    {
        Assert (values.size() == points.size(),
                ExcDimensionMismatch (values.size(), points.size()));
        for (unsigned int i=0; i<points.size(); ++i)
            values[i] = grad_pr_known<dim>::value (points[i]);
    }
    ...
    advection<dim>::value (const Point<dim> &p) const
    {
        Point<dim> value;
        const double time = this->get_time();
        value[0] = p[0]*(right-p[0])*p[1];
        value[1] = -time-p[0]*p[1]*p[1];
        
        return value;
    }
    template <int dim>
    void
    advection<dim>::value_list (const std::vector<Point<dim> > &points,
                                     std::vector<Tensor<1,dim> >    &values) const
    {
        Assert (values.size() == points.size(),
                ExcDimensionMismatch (values.size(), points.size()));
        for (unsigned int i=0; i<points.size(); ++i)
            values[i] = advection<dim>::value (points[i]);
    }
    ...
    void FullMovingMesh<dim>::assemble_system_rock ()
    {
        ...
        for (; cell!=endc; ++cell, ++pf_cell, ++phi_cell, ++vf_cell)
        {
            ...
            fe_values_pf[velocities].get_function_values (solution_pf, grad_pf_values);
    ...
    void FullMovingMesh<dim>::assemble_system_phi ()
    {
        ...
        advection<dim>                   advection_values;
        phi_boundary_values.set_time(time);
        advection_values.set_time(time);
...
//
////                    fe_face_values_rock.get_function_values (solution_rock, face_vr_values);
//                    advection_values.value_list (fe_face_values_rock.get_quadrature_points(), face_vr_values);
//
//                    for (unsigned int q_point=0; q_point<n_face_q_points; ++q_point)
...
//                            {
//                                for (unsigned int j=0; j<dofs_per_cell; ++j)
//                                    cell_matrix(i,j) -= (face_vr_values[q_point] *
...
//                }
    ...
    void FullMovingMesh<dim>::assemble_rhs_phi ()
    {
        ...
        advection<dim>                   advection_values;
        pr_known<dim>                    pr_known_values;
        grad_pr_known<dim>               grad_pr_known_values;
        RightHandSide<dim>               right_hand_side;
        advection_values.set_time(time);
        pr_known_values.set_time(time);
        grad_pr_known_values.set_time(time);
...
//                }
    ...
    template <int dim>
    void FullMovingMesh<dim>::compute_errors () const
    {
        ...
        const QIterated<dim> quadrature (q_trapez, fe_phi.degree+2);
