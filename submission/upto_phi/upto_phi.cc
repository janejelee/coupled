...
namespace FullSolver
{
    ...
    namespace data
    {
        ...
        const double timestep_size = 0.0000005;
    ...
    {
        values(0) = 0;
        values(1) = rocktopstress + sin(p[0]*PI/right);
        values(2) = 0;
    }
    ...
    {
        values(0) = 0.0;
        values(1) = 0.0;//p[1] + sin(PI*p[0]/right);
    }
            ...
            for (unsigned int q=0; q<n_q_points; ++q)
            {
                ...
                for (unsigned int i=0; i<dofs_per_cell; ++i)
                {
                    ...
                    local_rhs(i) +=  (
                                       ...
                                       * unitz_values[q] +
                                       (pf_values[q]*grad_phi_values[q] + phi_values[q]*grad_pf_values[q])
                                       )* phi_u[i]
        ...
        for (; cell!=endc; ++cell, ++vr_cell, ++phi_cell)
        {
            ...
            for (unsigned int q=0; q<n_q_points; ++q)
            {
                ...
                for (unsigned int i=0; i<dofs_per_cell; ++i)
                {
                    ...
                    local_rhs(i) += phi_i_p *
                    (div_vr_values[q] // commenting the rest for k constant
                     //                                             -
                ...
                //                std::cout << div_vr_values[q] << std::endl;
            }
    ...
    void FullMovingMesh<dim>::assemble_rhs_phi ()
    {
        ...
        for (; cell!=endc; ++cell, ++rock_cell, ++pf_cell, ++T_cell)
        {
            ...
            {
                const double k = 1.0;
                grad_pf_values[q_point] = -1./k * u_values[q_point];
