...
namespace FullSolver
{
    ...
    namespace data
    {
        ...
        const double timestep_size = 0.000002;
        const double chi = 0.1/timestep_size;
        const int total_timesteps = 75;
...
//        const double time = this->get_time();
        ...
        values(0) = 0.0;
        values(1) = sin(PI*p[0]/right);//400*p[1]*time;//p[1] + sin(PI*p[0]/right);
    }
...
//        const double time = this->get_time();
    ...
    void FullMovingMesh<dim>::setup_dofs_fluid ()
    {
        ...
        DoFTools::make_zero_boundary_constraints(dof_handler_pf, 2, constraints_pf, vector_field_mask);
...
//        }
    ...
    void FullMovingMesh<dim>::assemble_system_vf ()
    {
        ...
        for (; cell!=endc; ++cell, ++pf_cell, ++vr_cell, ++phi_cell)
        {
            ...
            for (unsigned int q_point=0; q_point<n_q_points; ++q_point)
            {
                ...
                //                std::cout << lambda/phi_values[q_point] << std::endl;
            }
    ...
    void FullMovingMesh<dim>::assemble_rhs_phi ()
    {
        ...
        for (; cell!=endc; ++cell, ++rock_cell, ++pf_cell, ++T_cell)
        {
            ...
            for (unsigned int q_point=0; q_point<n_q_points; ++q_point)
            {
                const double k = pow(old_phi_values[q_point],a)*pow(1./phi0,a);
