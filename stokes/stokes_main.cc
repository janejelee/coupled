...
namespace Step31
{
  ...
  namespace EquationData
  {
                                          ...
                                          const unsigned int component) const
    {
      ...
      static const Point<dim> source_centers[1]
        = {
    ...
    typename Triangulation<dim>::cell_iterator
    	          ...
    	          for (; cell!=endc; ++cell)
    	            for (unsigned int face_number=0;
    	                 ...
    	                 ++face_number)
    	              if ((std::fabs(cell->face(face_number)->center()(1) - (1.0)) < 1e-12)) // if x=1.0, RHS boundary
    	                cell->face(face_number)->set_boundary_id (1);
    	              else if ((std::fabs(cell->face(face_number)->center()(1) - (0.0)) < 1e-12)) // if y=-0.5, bottom boundary
