...
//									  -data::lambda * data::perm_const * data::rho_f * (1.0/data::phi)*p[1]*p[1];
  ...
  MixedLaplaceProblem<dim>::MixedLaplaceProblem (const unsigned int degree)
	...
	vf_degree (degree),
	vf_fe (FE_RaviartThomas<dim>(pf_degree), 1),
...
//    
    ...
    //    USE THIS NUMBERING OF DOFS FOR RT SPACES
        ...
        std::vector<types::global_dof_index> vf_dofs_per_component (dim);
        DoFTools::count_dofs_per_component (vf_dof_handler, vf_dofs_per_component);
        
        const unsigned int n_u = pf_dofs_per_component[0],
                           n_pf = pf_dofs_per_component[dim],
    					   n_vf = vf_dofs_per_component[0];
	  ...
	  QGauss<dim>   quadrature_formula(vf_degree+2);
	     ...
	     for (; cell!=endc; ++cell, ++pf_cell)
    	 	...
    	 	for (unsigned int q=0; q<n_q_points; ++q)
    	 	          for (unsigned int i=0; i<dofs_per_cell; ++i)
    	 	            {
    	 	              const Tensor<1,dim> phi_i_u     = vf_fe_values[velocities].value (i, q);

    	 	       
    	 	              local_rhs(i) += (-phi_i_u* data::lambda*data::perm_const*(1.0/data::phi)* data::rho_f* unitz_values[q]  
            		              + 0.0* phi_i_u * rock_vel_values[q]  // rock velocity added
            		                  - phi_i_u *  data::lambda*data::perm_const*(1.0/data::phi)* grad_pf_values[q])*
    	 	                              pf_fe_values.JxW(q);
    	 	            }
// 
//          for (unsigned int i=0; i<dofs_per_cell; ++i)
