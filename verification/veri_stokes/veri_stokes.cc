...
//        const double k = 1.0;
    ...
    template <int dim>
    class ExactFunction_phi : public Function<dim>
    {
    public: ExactFunction_phi () : Function<dim>() {}
        virtual double value (const Point<dim>   &p, const unsigned int  component = 0) const;
    };
    
    template <int dim>
    class ExactFunction_vf : public Function<dim>
    {
    public: ExactFunction_vf () : Function<dim>(dim) {}
        virtual void vector_value (const Point<dim> &p, Vector<double>   &value) const;
    };
...
//        values(1) = -p[1]*p[1] + lambda * k /phi *(p[0]*(1-p[1]*p[1]) - rho_f);
    ...
    template <int dim>
    class ExactFunction_rock : public Function<dim>
    {
    public: ExactFunction_rock () : Function<dim>(dim+1) {}
        virtual void vector_value (const Point<dim> &p, Vector<double> &value) const;
    };
...
//                                                      fe_rock.component_mask(velocities));
    ...
    void FullMovingMesh<dim>::compute_errors () const
    {
        ...
        ExactFunction_rock<dim> exact_solution_rock;
    ...
    void FullMovingMesh<dim>::run ()
    {
        ...
        ExactFunction_phi<dim> phi_function;
        phi_function.set_time (time);
        
        ExactFunction_vf<dim> vf_function;
        vf_function.set_time (time);
        
        VectorTools::interpolate(dof_handler_pf, ExactFunction_pf<dim>(), solution_pf);
        ...
        std::cout << "   Assembling for rock..." <<  std::endl << std::flush;
