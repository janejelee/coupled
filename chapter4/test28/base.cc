...
namespace FullSolver
{
    ...
    namespace data
    {
        ...
        const double R = 4.6;
        const double rocktopstress = -0.01;
        const double sill_temp = 50000;
        const double q = 10.0;
        const double a = 8.0;
        
        const unsigned int test_no = 27;
        
        const double mur0 = 1e19;
        const double d = 1e3;
    ...
    {
        values(0) = 0; //ux
        values(1) = 2*1; //uy = -gradpfpz
        values(2) = patm + 2*(top-p[1]); //p
    }
...
//            no_normal_flux_boundaries.insert (2);
    ...
    void FullMovingMesh<dim>::run ()
    {
        ...
        while (timestep_number < total_timesteps)
        {
            ...
            << "            RL"<< refinement_level <<" using normal phi ICs as 0.7. Vf BEING solved. All equations in phi as usual. lambda " << lambda << ". Timestep size is " << timestep_size << ". H0 is " << H0 << ". Test Number " << test_no << std::endl
            << "===========================================" << std::endl;
