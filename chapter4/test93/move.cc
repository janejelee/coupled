...
namespace FullSolver
{
    ...
    namespace data
    {
        ...
        const double sill_temp = 10000;
        const double q = 10.0;
        const double a = 8.0;
        
        const unsigned int test_no = 92;
        
        const double mur0 = 1e20;
        const double d = 1e3;
        const double H0 = 1e-10; // 1e-10 corr to 1/3ma.
        ...
        const double Re = 1e3*d*H0/mur0; //depends on mu_r
        const double Ri = 10*d/(H0*H0); //always
        const double Nu = Re * Pr;
        const double gamma = 7.27e-4*d; // depends on compaction coefficient
        const double alpha = mur0*H0/(1e4*d*d);
        
        const double timestep_size = 0.01;
        const int total_timesteps = 3000;
        
        const double sed_rate = 100000;
        const double sed_height = timestep_size*sed_rate;
        
        const int sill_time = 100;
    }
    ...
    double RockTopStress<dim>::value (const Point<dim>  &p, const unsigned int /*component*/) const
    {
        ...
        if (time < 0.1)
            ...
            return rocktopstress +  0*rocktopstress*std::sin(p[0]/10) ;
    }
        ...
        for (; cell!=endc; ++cell, ++pf_cell, ++phi_cell, ++vf_cell)
        {
            ...
            for (unsigned int q=0; q<n_q_points; ++q)
            {
                ...
                for (unsigned int i=0; i<dofs_per_cell; ++i)
                {
                    ...
                    {
                        local_matrix(i,j) += (- 2 * alpha/ phi_values[q] * (1-phi_values[q])*
                    ...
                    local_rhs(i) +=  (
                                       ...
                                       * unitz_values[q] +
                                       (pf_values[q]*grad_phi_values[q] +
                                        phi_values[q]* -u_values[q]) // for grad pf = -u
                                       )* phi_u[i]
    ...
    void FullMovingMesh<dim>::assemble_system_pf ()
    {
        ...
        for (; cell!=endc; ++cell, ++vr_cell, ++phi_cell)
        {
            ...
            for (unsigned int q=0; q<n_q_points; ++q)
            {
                const double perm = 1;// pow(phi_values[q]/phi0,a);
                ...
                for (unsigned int i=0; i<dofs_per_cell; ++i)
                {
                    ...
                    local_rhs(i) += phi_i_p * (div_vr_values[q] - 0*lambda*rho_f*unitz_values[q][component_i]
                                               *grad_phi_values[q][component_i]*a*pow(phi_values[q],a-1.0)
                                               *( 1./pow(phi0,a) )
                                               )
                    * fe_values_pf.JxW(q);
                }
        ...
        for (; cell!=endc; ++cell, ++pf_cell, ++vr_cell, ++phi_cell)
        {
            ...
            for (unsigned int q_point=0; q_point<n_q_points; ++q_point)
            {
                for (unsigned int i=0; i<dofs_per_cell; ++i)
                {
                    ...
                    const double perm = 1;//pow(phi_values[q_point],a)*pow(1./phi0,a);
    ...
    void FullMovingMesh<dim>::assemble_rhs_phi ()
    {
        ...
        for (; cell!=endc; ++cell, ++rock_cell, ++pf_cell, ++T_cell)
        {
            ...
            for (unsigned int q_point=0; q_point<n_q_points; ++q_point)
            {
                ...
                double pl = 0.0;
                pl = 1.0 + (top -  fe_values_phi.quadrature_point(q_point)(1) ) *
                (rho_r + old_phi_values[q_point]*(rho_f - rho_r) );
                ...
                for (unsigned int i=0; i<dofs_per_cell; ++i)
                {
                    cell_rhs(i) += ( (fe_values_phi.shape_value(i,q_point)
                                      + delta *(vr_values[q_point] *
                                                fe_values_phi.shape_grad(i,q_point)) )
                                    *(
                                      - Re*Ri*chem_c*old_phi_values[q_point]/mu_r * (pr_values[q_point] - pf_values[q_point])
                                      - phi0* gamma * ( ddt + vr_values[q_point]*(grad_pr_values[q_point] - grad_pf_values[q_point])) *
                ...
                for (unsigned int i=0; i<dofs_per_cell; ++i)
                {
                    for (unsigned int j=0; j<dofs_per_cell; ++j)
                    {
                        cell_matrix_nontime(i,j) += ( (
                                                     ...
                                                     fe_values_T.shape_value(i,q_point)
                                                     +
                                                     kappa/(phi0*Nu/phi_values[q_point])
        ...
        for (; cell!=endc; ++cell)
        {
            ...
            if (timestep_number == sill_time || timestep_number == sill_time + 1)
            {
                ...
                for (unsigned int q_point=0; q_point<n_q_points; ++q_point)
                {
                    for (unsigned int i=0; i<dofs_per_cell; ++i)
                    {
                        cell_rhs(i) += ( (sill_source_values[q_point]/ (phi0*Nu) *
                                          fe_values_T.shape_value(i,q_point) *
                                          fe_values_T.JxW(q_point));
                                        ...
                                        void FullMovingMesh<dim>::solve_phi ()
                                        {
                                            ...
                                            //        MatrixTools::apply_boundary_values (boundary_values, system_matrix_phi, solution_phi, system_rhs_phi);
                                            //
                                        ...
                                        void FullMovingMesh<dim>::output_results ()  const
                                        {
                                            ...
                                            filename_rock << "solution_rock"+ Utilities::int_to_string(test_no) +"-" + Utilities::int_to_string(timestep_number, 3) + ".vtk";
                                            ...
                                            const std::string filename_fluid = "solution_fluid-"+ Utilities::int_to_string(test_no) +"-"
                                            ...
                                            const std::string filename_phi = "solution_phi-"+ Utilities::int_to_string(test_no) +"-"
                                            + Utilities::int_to_string(timestep_number, 3) + ".vtk";
                                            ...
                                            const std::string T_filename = "T_solution-" + Utilities::int_to_string(test_no) +"-"
                                            + Utilities::int_to_string(timestep_number, 3) + ".vtk";
                                        ...
                                        void FullMovingMesh<dim>::run ()
                                        {
                                            ...
                                            while (timestep_number < total_timesteps)
                                            {
                                                ...
                                                << "            RL"<< refinement_level <<" using normal phi ICs as 0.7. Vf BEING solved. All equations in phi as usual. lambda " << lambda << ". Timestep size is " << timestep_size << ". H0 is " << H0 << " Test no " << test_no << std::endl
                                                << "===========================================" << std::endl;
