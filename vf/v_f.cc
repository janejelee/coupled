...
namespace Step20
{
  ...
  namespace data
  {
      ...
      const double coeff = lambda*k/phi;
                   ...
                   std::vector<Tensor<1, dim> >   &values)
    {
        ...
        for (unsigned int point_n = 0; point_n < points.size(); ++point_n)
        {
            ...
            values[point_n][0] = -(points[point_n][1]*points[point_n][1] + 1.0
                                    - 3* points[point_n][0]*points[point_n][0]);
            values[point_n][1] = -2*points[point_n][0]*points[point_n][1];
        }
                                    ...
                                    Vector<double>   &values) const
  {
      ...
      values(1) = -p[0]*p[1] + data::coeff*2*p[0]*p[1] - data::coeff*data::rho_f;
  }
          ...
          for (unsigned int i=0; i<dofs_per_cell; ++i)
          {
              ...
              for (unsigned int q_point=0; q_point<n_q_points; ++q_point)
                local_rhs(i) += (fe_values.shape_value(i,q_point) *
                                gravity_values[q_point][component_i] +
                                 fe_values.shape_value(i,q_point) *
                                 rock_vel_values[q_point][component_i]
                                 -data::coeff* fe_values.shape_value(i,q_point) *
                                 pressure_flux_values[q_point][component_i]
                                 ) *
                                fe_values.JxW(q_point);
