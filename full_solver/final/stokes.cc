...
//        values(2) = 3/2*p[1]*p[1]-2*p[1];
    ...
    void FullMovingMesh<dim>::assemble_system_rock ()
    {
        ...
        ExactSolution_phi<dim>     phi_solution;
        phi_solution.set_time(time);
...
//            fe_values_phi.get_function_values (solution_phi, phi_values);
    ...
    void FullMovingMesh<dim>::assemble_system_phi ()
    {
        ...
        FEValues<dim> fe_values (fe_phi, quadrature_formula,
                                 update_values    |  update_gradients |
                                 update_quadrature_points  |  update_JxW_values);
        //        FEFaceValues<dim> fe_face_values (fe_phi, face_quadrature_formula,
    ...
    void FullMovingMesh<dim>::assemble_system_T ()
    {
        ...
        FullMatrix<double>   cell_matrix_nontime (dofs_per_cell, dofs_per_cell);
        ...
        for (; cell!=endc; ++cell, ++phi_cell, ++vr_cell, ++vf_cell)
        {
            ...
            for (unsigned int q=0; q<n_q_points; ++q)
            {
                double coeff_r = rho_r*c_r*(1-phi_values[q]);
                double coeff_r_old = rho_r*c_r*(1-old_phi_values[q]);
                ...
                for (unsigned int i=0; i<dofs_per_cell; ++i)
                {
                    for (unsigned int j=0; j<dofs_per_cell; ++j)
                    {
                        cell_matrix_nontime(i,j) += (
                          ...
                          cell_matrix_old(i,j) += ( coeff_r_old*
                                             fe_values_T.shape_value(i,q)*
                                             fe_values_T.shape_value(j,q)
                                             ) *
                                            fe_values_T.JxW(q) ;
...
//        std::cout << "   Solving for v_f..." << std::endl;
    ...
    void FullMovingMesh<dim>::solve_phi ()
    {
        ...
        system_rhs_phi.add(timestep_size,nontime_rhs_phi);
        
        system_matrix_phi.copy_from(mass_matrix_phi);
        system_matrix_phi.add(timestep_size,nontime_matrix_phi);
        
        std::map<types::global_dof_index,double> boundary_values;
        ExactSolution_phi<dim> diri_values;
        ...
        SparseDirectUMFPACK  A_direct;
        A_direct.initialize(system_matrix_phi);
        A_direct.vmult (solution_phi, system_rhs_phi);
