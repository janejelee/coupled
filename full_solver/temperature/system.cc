...
//                                                                      rock_constraints);
    ...
    void DAE<dim>::setup_T_dofs ()
    {
        ...
        DoFRenumbering::Cuthill_McKee (T_dof_handler);
        
        T_hanging_node_constraints.clear ();
        DoFTools::make_hanging_node_constraints (T_dof_handler,
                                                 T_hanging_node_constraints);
        ...
        T_hanging_node_constraints.condense (dsp);
...
//        std::cout << "   Solving for v_f..." << std::endl;
    ...
    template <int dim>
    void DAE<dim>::assemble_T_system ()
       {
           T_system_matrix = 0;
           T_nontime_matrix = 0;
           MatrixCreator::create_mass_matrix(T_dof_handler,
                                             QGauss<dim>(data::problem_degree+3),
                                             T_mass_matrix);
           ...
           for (; cell!=endc; ++cell)
           {
               ...
               for (unsigned int q_point=0; q_point<n_q_points; ++q_point)
                   ...
                   {
                       for (unsigned int j=0; j<dofs_per_cell; ++j)
                           cell_matrix(i,j) += ((T_fe_values.shape_grad(i,q_point) *
                        		   	   	   	   	   T_fe_values.shape_grad(j,q_point)
                                                 ) *
                        		   	   	   	   	   	 T_fe_values.JxW(q_point));
                   }
       ...
       void DAE<dim>::assemble_T_rhs ()
       {
           ...
           for (; cell!=endc; ++cell)
           {
               ...
               for (unsigned int q_point=0; q_point<n_q_points; ++q_point)
                   for (unsigned int i=0; i<dofs_per_cell; ++i)
                   {
                       cell_rhs(i) += (T_fe_values.shape_value(i,q_point) *
                                       rhs_values [q_point] *
									   T_fe_values.JxW(q_point));
                   }
                       ...
                       for (unsigned int q_point=0; q_point<n_face_q_points; ++q_point)
                       {
                           ...
                           for (unsigned int i=0; i<dofs_per_cell; ++i)
                               cell_rhs(i) += (neumann_value *
                            		   	   	   T_fe_face_values.shape_value(i,q_point) *
										   	   T_fe_face_values.JxW(q_point));
                       }
                   }
    ...
    void DAE<dim>::solve_T ()
    {
        ...
        T_direct.initialize(T_system_matrix);
        T_direct.vmult (T_solution, T_system_rhs);
    }
...
//        << ". " << std::endl;
    ...
    void DAE<dim>::output_phiT_results ()
    {
        ...
        data_out_phi.attach_dof_handler(phi_dof_handler);
        data_out_phi.add_data_vector(phi_solution, "U");

        data_out_phi.build_patches();

        const std::string phi_filename = "phi_solution-"
        + Utilities::int_to_string(timestep_number, 3) +
        ".vtk";
        std::ofstream output_phi(phi_filename.c_str());
        data_out_phi.write_vtk(output_phi);
        
        
        DataOut<dim> data_out_T;
        
        data_out_T.attach_dof_handler(T_dof_handler);
        data_out_T.add_data_vector(T_solution, "T");
