...
namespace Step22
{
    ...
    template <int dim>
    class BoundaryValuesSides : public Function<dim>
    {
    public:
      BoundaryValuesSides () : Function<dim>(dim+1) {}
      virtual double value (const Point<dim>   &p,
                            const unsigned int  component = 0) const;
      virtual void vector_value (const Point<dim> &p,
                                 Vector<double>   &value) const;
    ...
    template <int dim>
    double
    BoundaryValuesSides<dim>::value (const Point<dim>  &p,
                                const unsigned int component) const
    {
			if (component == 0)
    ...
    template <int dim>
    void
    BoundaryValuesSides<dim>::vector_value (const Point<dim> &p,
                                       Vector<double>   &values) const
    {
      for (unsigned int c=0; c<this->n_components; ++c)
        values(c) = BoundaryValuesSides<dim>::value (p, c);
    }
...
//
    ...
    template <int dim>
    void
    StokesProblem<dim>::error_analysis () 
	{
